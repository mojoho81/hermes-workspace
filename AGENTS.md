...
- Deploy/bootstrap: `scripts/deploy.sh` (idempotent; `--dry-run` to preview, `--no-restart` to skip the service bounce). It validates tools, runtime symlinks, locked deps, build, systemd unit, ports 3300/8642/9119, Workspace enhanced capabilities (sessions, skills, memory, config, jobs), and fork parity as a separate provenance check. For fresh-VPS restore order (runtime from encrypted backup first, then clone, .env, unit, deploy) see the script header. `/root/hermes-runtime` stays out of git and restores from encrypted backups only.
